...
-- unhealthy days by monitoring station count and years of data.
-- Raw unhealthy day counts favor counties with more monitors
-- and more years of data, this view provides a fairer
-- county-level comparison.
--
-- avg_unhealthy_days_per_year answers:
--   "On average, how many days per year did a typical monitoring
--    station in this county exceed the federal PM2.5 standard?"
--
-- Latitude and longitude are averaged across all stations in
-- the county for map visualization purposes.
-- ============================================================
DROP VIEW IF EXISTS county_unhealthy_avg;

CREATE VIEW county_unhealthy_avg AS
SELECT 
    m.county_name,
    AVG(m.latitude) AS latitude,
    AVG(m.longitude) AS longitude,
    COUNT(*) AS total_unhealthy_days,
    COUNT(DISTINCT m.site_id) AS monitor_count,
    COUNT(*) / COUNT(DISTINCT m.site_id) AS total_per_monitor,
    COUNT(*) / COUNT(DISTINCT m.site_id) / 24 AS avg_unhealthy_days_per_year
FROM daily_readings d
JOIN monitors m ON d.site_id = m.site_id
WHERE d.arithmetic_mean > 35
AND YEAR(d.date_local) BETWEEN 1999 AND 2022
...
SELECT * FROM county_unhealthy_avg 
ORDER BY avg_unhealthy_days_per_year DESC;